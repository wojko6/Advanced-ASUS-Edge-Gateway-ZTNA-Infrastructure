...
1. Repeat the exit-node packet/counter correlation on GNUton `3004.388.11_1-gnuton1_tuf` so the current-firmware claim is backed by a fresh before/after-NAT datapath observation rather than only the 2026-09-22 historical artifact.
2. Repeat the classic-DNS Tailscale datapath correlation on the new firmware if a current-firmware AUDIT-03-equivalent claim is required. A successful lookup or health check alone does not establish the complete packet path.
...
As of 2026-09-23, the reference router runs GNUton `3004.388.11_1-gnuton1_tuf` with the `v2.1.4-dev` project deployed. The same-day reboot, core health and USB audits, both admin rules, Fedora DNS/verified HTTPS, operator-reported Android admin access, and the distinct unauthorized-tailnet management denial all passed within their documented limits. Requirement F-01 now has both authorized and unauthorized role evidence for the tested deployment. AUDIT-02/03 retain their 2026-09-22 live-validation scope; the new firmware has not yet received an equivalent full exit-node/DNS packet-correlation rerun. Diversion `Large + snbAdSupport=no` remains under normal-use observation rather than a permanently accepted filtering baseline. Next: revalidate the exit-node datapath on the current firmware, then close the remaining current-firmware DNS/reboot/normal-use evidence gaps before promoting broader security or stability claims.
