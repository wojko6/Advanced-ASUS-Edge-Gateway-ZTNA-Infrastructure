...
- **2026-09-25 — Xbox Cloud Gaming in Microsoft Edge / WebRTC:** a Fedora/Wayland client running Edge 154.0.4258.37 recorded approximately 1000 seconds of xCloud WebRTC telemetry. The inbound stream remained 1920 x 1080 H.264, 935/1000 frame-rate samples were 58–62 FPS, and all retained samples reported zero video packet loss, dropped frames and freezes. The selected ICE pair used UDP without a relay candidate and reported 30–34 ms WebRTC RTT. Generic Edge hardware-video-decode capability was present, while the active stream reported `decoderImplementation=FFmpeg` and `powerEfficientDecoder=false`; the evidence therefore does not claim CPU-only decode or proven hardware decode for the stream. See the [sanitized validation](2026-09-25/xbox-cloud-edge-webrtc-validation.md) and [case study](../docs/xbox-cloud-edge-webrtc-case-study.md).
- **2026-09-25 — router live checkpoint:** GNUton `3004.388.11_1-gnuton1_tuf` remained healthy with SSD-backed Entware/data mounts, active swap, core services, DNSSEC-validating Unbound and a project health result of `0 failure(s), 0 warning(s)`. Controlled Fedora probes produced exact production counter deltas for LAN UDP/TCP 53 interception, direct TCP/853 rejection and Tailscale UDP/53 interception. A new Exit Node capture was not completed, so the 2026-09-23 fixed-flow evidence remains authoritative for that datapath. See the [sanitized checkpoint](2026-09-25/router-live-checkpoint.md).
- **2026-09-25 — GeForce NOW Ethernet vs Wi-Fi 6 stability:** a Fedora client compared Gigabit Ethernet with 5 GHz HE/802.11ax through the same ASUS gateway to `Poland (NP-WAW-01)`. Both captured GeForce NOW overlay snapshots reported zero packet loss and zero frame loss. The matched short baseline showed materially higher Wi-Fi jitter, and the longer Wi-Fi sample recorded rare local-path excursions up to 247 ms to the gateway and 311 ms to a public Internet target while still receiving every ICMP probe. A recovered Wi-Fi capture provided 3,675,119 readable packets across 678.772 s; UDP/5004 accounted for approximately 99.3% of the readable packets. The file was truncated when the capture tmpfs reached capacity, so the Wi-Fi PCAP is used only for bounded flow/volume/timing observations and not as an application packet-loss source. See the [sanitized validation](2026-09-25/geforce-now-ethernet-vs-wifi6-validation.md) and [case study](../docs/geforce-now-ethernet-vs-wifi6-case-study.md).
- **2026-09-23 — unauthorized tailnet management denial:** a distinct Windows tailnet client outside the configured admin-source allowlist retained Tailscale peer reachability but could not establish TCP/8443 to router management. A directional `tailscale0` capture recorded five client-to-router SYN packets, while a temporary source-specific counter-only firewall rule matched the same five packets / 260 bytes before returning to the unchanged production deny tail. The test instrumentation was removed and the final production health check returned zero failures and warnings. See the [sanitized validation](2026-09-23/unauthorized-tailnet-management-denial.md).
- **2026-09-23 — post-firmware AUDIT-02 Exit Node revalidation:** after the GNUton `3004.388.11_1-gnuton1_tuf` upgrade, a Fedora client used the ASUS as its active exit node and sent a five-packet fixed-ID ICMP flow. Simultaneous captures correlated identifier `4244`, sequence numbers `1..5`, and matching replies on `tailscale0` before NAT and `ppp0` after source translation. The client received 5/5 replies, both captures reported zero kernel drops, and the final production health check returned zero failures and warnings. Raw Tailscale/WAN addresses are omitted. See the [sanitized post-firmware revalidation](2026-09-23/audit-02-post-firmware-exit-node-revalidation.md).
- **2026-09-22 — production DoT/853 blocking live validated:** PR #58 was deployed to the reference router with `EDGE_BLOCK_LAN_DOT=1`. The live health check returned `0 failure(s), 0 warning(s)` and `HEALTHCHECK_RC=0`; a Fedora connection to `8.8.8.8:853` over the normal LAN path failed with `Connection refused`, while the managed production reject rule recorded 1 packet / 60 bytes. See the [sanitized production validation](2026-09-22/dot-853-block-production-validation.md).
- **2026-09-22 — DoT/853 blocking prototype A/B/A:** Fedora on the normal LAN path established TLS to `8.8.8.8:853` at baseline; a temporary `br0` FORWARD rule then rejected TCP/853 and recorded 1 packet / 60 bytes; after rollback, the TLS connection succeeded again. See the [sanitized DoT prototype validation](2026-09-22/dot-853-block-prototype-validation.md).
- **2026-09-22 — production LAN classic-DNS enforcement live validated:** PR #57 was deployed to the reference router with `EDGE_ENFORCE_LAN_DNS=1`. The production health check returned `0 failure(s), 0 warning(s)` and `HEALTHCHECK_RC=0`; subsequent Fedora UDP/TCP queries explicitly addressed to `8.8.8.8:53` traversed the normal LAN gateway and increased the managed external redirect counters to 6 UDP packets / 480 bytes and 6 TCP packets / 360 bytes. See the [sanitized production validation](2026-09-22/lan-dns-enforcement-production-validation.md).
...
| Browser / WebRTC | Edge WebRTC Internals, browser GPU capability export | browser-visible RTP, codec, frame, ICE and decoder-path observations for the defined session |
