...
- **2026-09-25 — GeForce NOW Ethernet vs Wi-Fi 6 stability:** a Fedora client compared Gigabit Ethernet with 5 GHz HE/802.11ax through the same ASUS gateway to `Poland (NP-WAW-01)`. Both captured GeForce NOW overlay snapshots reported zero packet loss and zero frame loss. The matched short baseline showed materially higher Wi-Fi jitter, and the longer Wi-Fi sample recorded rare local-path excursions up to 247 ms to the gateway and 311 ms to a public Internet target while still receiving every ICMP probe. The Wi-Fi packet capture was unavailable, so packet-level stream analysis is limited to Ethernet. See the [sanitized validation](2026-09-25/geforce-now-ethernet-vs-wifi6-validation.md) and [case study](../docs/geforce-now-ethernet-vs-wifi6-case-study.md).
