...
- Optional `EDGE_BLOCK_LAN_DOT=1` adds a dedicated `filter/EDGE_LAN_DOT_FORWARD` chain and places the `EDGE_LAN_IF -> EDGE_LAN_DOT_FORWARD` jump immediately after the project-owned Tailscale FORWARD jump, before platform FORWARD rules. The managed chain rejects TCP/`EDGE_DOT_PORT` (default 853) with `tcp-reset`.
- Blocking TCP/853 is a DoT-specific control. It may break clients configured for strict/private DNS and may cause software to try other encrypted resolver transports. It does not block DoH/HTTPS, DoQ/QUIC, VPN-carried DNS, or application-specific encrypted DNS.
